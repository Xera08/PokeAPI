...
class JetsnackNavController(
    ...
    fun navigateToPokemonDetail(pokemonId: Short, origin: String, from: NavBackStackEntry) {
        // In order to discard duplicated navigation events, we check the Lifecycle
        if (from.lifecycleIsResumed()) {
            navController.navigate("${MainDestinations.SNACK_DETAIL_ROUTE}/$pokemonId?origin=$origin")
        }
    }
}
